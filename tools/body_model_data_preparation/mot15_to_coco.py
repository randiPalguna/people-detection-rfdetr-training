...
def convert_mot15_to_coco(seq_dir: str, output_dataset_dir: str, target_fps: float = 5.0):
    """Mengonversi MOT15 ke format COCO JSON dan menyalin gambar ke folder Roboflow-ready."""
    ...
    coco_data = {
        "info": {"description": f"MOT15 {seq_path.name} converted to COCO (Subsampled {target_fps} FPS)"},
    ...
    with open(gt_file, "r") as f:
        for line in f:
            ...
            coco_data["annotations"].append({
                ...
                "track_id": obj_id, # Menyimpan ID tracking
                "iscrowd": 0
            })
            ann_id += 1
    ...
    print(f"   Total Anotasi: {len(coco_data['annotations'])}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert MOT15 to Roboflow-ready COCO and Visualize")
    parser.add_argument("seq_dir", help="Path ke folder sequence MOT15 asli (ex: /data/MOT15/train/PETS09-S2L2)")
    ...
    json_result_path, new_dataset_path = convert_mot15_to_coco(
